...
export const processComputeAllCountryEvents = async (event) => {
    ...
    console.log('inside processComputeAllCountryEvents');
    ...
    console.log('inside processComputeAllCountryEvents 1', list);
    ...
    console.log('inside processComputeAllCountryEvents 2');
    ...
    let flagRangeFileNotFound = false;
    if(range){
        let userRangeFileKey = projectid + '_' + userprofileid + '_' + year + '_' + role +'_calendar_range_job_enc.json'
        console.log('userRangeFileKey', userRangeFileKey)
        var command = new GetObjectCommand({
          Bucket: BUCKET_NAME,
          Key: userRangeFileKey,
        });
        ...
        if(!flagRangeFileNotFound){
            console.log()
            calendarList.push(projectid + '_' + userprofileid + '_' + year + '_' + role +'_calendar_range_job')
        }
    }else if(month != "00"){
    ...
    if(month == "00" || flagMonthlyFileNotFound || flagRangeFileNotFound){
        ...
        if(!flagUserFileNotFound){
            ...
            try {
                responseS3 = await s3Client.send(command);
                console.log('inside processComputeAllCountryEvents 3a');
                const s3ResponseStream = responseS3.Body;
                console.log('inside processComputeAllCountryEvents 3b');
                const chunks = []
                for await (const chunk of s3ResponseStream) {
                    chunks.push(chunk)
                }
                console.log('inside processComputeAllCountryEvents 3c');
                const responseBuffer = Buffer.concat(chunks)
                console.log('inside processComputeAllCountryEvents 3d');
                let decryptedData = await processDecryptData(projectid, responseBuffer.toString())
                const jsonContent = JSON.parse(decryptedData);
                console.log('inside processComputeAllCountryEvents 3e');
            ...
            // console.log('inside processComputeAllCountryEvents 4', storedManifest);
    ...
    while(cnt < calendarList.length) {
        ...
        console.log('cnt', cnt, calendarList[cnt]);
        ...
        try {
          ...
          console.error('err1',err, command);
          flagEncryptedNotFound = true
        }
        if(flagEncryptedNotFound){
            command = new GetObjectCommand({
                        ...
                        for(var j = 0; j < locations.length; j++) {
                            ...
                            for(var l = 0; l < events.length; l++) {
                                ...
                                if(pushFlag) {
                                    ...
                                    if(assReports[mmddyyyy + ';' + entity + ';' + locations[j] + ';' + events[l].id] != null && !range) {
    ...
    const response = {statusCode: 200, body: {result: true, data: arrEvents, lastEvaluatedKey: cnt === calendarList.length ? null : cnt}};
